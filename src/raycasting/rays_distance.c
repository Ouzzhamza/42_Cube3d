...
/*   Updated: 2022/08/17 16:34:09 by mmoumni          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
double	calculate_ray_distance(t_point player_pos, t_point wall_pos)
{
	...
	// sqrt(pow((player_pos.x - wall_pos.x), 2) + pow((player_pos.y - wall_pos.y), 2));
...
int	draw_wall(t_raycast *raycast, int x_start, double wall_height)
{
	...
	i = 0;
	while ((i + wall_height_half) < wall_height && (i + wall_height_half) < WIN_HEIGHT && (i + wall_height_half) >= 0)
	{
